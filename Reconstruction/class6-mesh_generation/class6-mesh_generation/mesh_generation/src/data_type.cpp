...
////////////////////////////////////computeEdge//////////////////////////////////////////////////////////////////////////
void Mesh::computeEdge(){
		//--------------------------------compute edges -----------------------------------------//
	// get edges from facets
	Edges_.clear();
	foreach(QVector<int> facet, Facets_){
		int pt_num = facet.size();
		for (int i = 0; i < pt_num; i++){
			int id0 = facet[i];
			int id1 = facet[(i + 1) % pt_num];
			if(id0>id1)swap(id0, id1);
			if(Edges_.contains(qMakePair(id0, id1))) continue;
            Edges_<< qMakePair(id0, id1);
		}
	}
}

//=======================================saveMeshAsOBJ======================================//
bool Mesh::saveMeshAsOBJ(QString file_name){
	QFile file(file_name);
	if (!file.open(QIODevice::WriteOnly | QIODevice::Text)){
		return false;
	}
	QTextStream out(&file);
	out << "# This file uses format defined by IGIT group. " << endl;
	out << endl;

	out << "g default" << endl;

	// vertices
	foreach(Point v, Vertices_){
...
//=======================================saveMeshAsOFF======================================//
bool Mesh:: saveMeshAsOFF(QString file_name){
	QFile file(file_name);
	if (!file.open(QIODevice::WriteOnly | QIODevice::Text)){
		return false;
	}
	QTextStream out(&file);
	out<<"OFF"<<endl;
	out<<Vertices_.size()<<" "<<Facets_.size()<<" "<<0<<endl;

	foreach(Point pt, Vertices_){
