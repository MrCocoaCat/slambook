...
typedef TREAL **triangle;            /* TREALly:  typedef triangle *triangle   */
...
typedef TREAL **subseg;                  /* TREALly:  typedef subseg *subseg   */
...
/*   subsegment that isn't TREALly connected to a subsegment at that          */
/*   location.                                                               */
...
extern int triunsuitable(vertex triorg, vertex tridest, vertex triapex, TREAL area);
...
"    segments that meet (or intersect) at an extremely small angle, or if\n");
  printf(
"    such an angle is introduced by the -c switch.  If you don't TREALize\n");
  printf(
...
"  you have the X Windows system.  Sorry, Microsoft Windows users.\n");
  ...
  printf("\n");
  printf(
"  If you use Triangle, and especially if you use it to accomplish TREAL\n");
  printf(
...
/*  or subsegment that doesn't have a full complement of TREAL subsegments    */
/*  to point to.                                                             */
/*                                                                           */
...
/*  `dummytri' and `dummysub' as if they were TREAL mesh entities, with no    */
/*  harm done.                                                               */
/*                                                                           */
/*****************************************************************************/
               ...
               int subsegbytes)
{
  ...
  /*   values don't TREALly matter, as long as they can legally be          */
  /*   dereferenced.                                                       */
  ...
  if (b->usesegments) {
    ...
    /*   triangle side or subsegment end that isn't attached to a TREAL */
    /*   subsegment.                                                   */
    ...
    /*   operations, but their values don't TREALly matter, as long as they */
...
/*  floating-point arithmetic is too much like TREAL arithmetic.              */
/*                                                                           */
...
/*  are converted into TREAL triangles of the triangulation.  The procedure   */
/*  pulls each hull's bounding triangles apart, then knits them together     */
/*  like the teeth of two gears.  The Delaunay property determines, at each  */
/*  step, whether the next "tooth" is a bounding triangle of the left hull   */
/*  or the right.  When a bounding triangle becomes TREAL, its apex is        */
/*  changed from NULL to a TREAL vertex.                                      */
/*                                                                           */
/*  Only two new triangles need to be allocated.  These become new bounding  */
/*  triangles at the top and bottom of the seam.  They are used to connect   */
/*  the remaining bounding triangles (those that have not been converted     */
/*  into TREAL triangles) into a single fan.                                  */
/*                                                                           */
...
long divconqdelaunay(struct mesh *m, struct behavior *b)
{
  ...
  /* Discard duplicate vertices, which can TREALly mess up the algorithm. */
  i = 0;
...
int reconstruct(struct mesh *m, struct behavior *b, int *trianglelist,
                TREAL *triangleattriblist, TREAL *triangleaTREAList,
...

{
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    ...
    if (b->vararea) {
#ifdef TRILIBRARY
      area = triangleaTREAList[elementnumber - b->firstnumber];
#else /* not TRILIBRARY */
...

{
  ...
  if (b.refine) {
    ...
    m.hullsize = reconstruct(&m, &b, in->trianglelist,
                             in->triangleattributelist, in->triangleaTREAList,
