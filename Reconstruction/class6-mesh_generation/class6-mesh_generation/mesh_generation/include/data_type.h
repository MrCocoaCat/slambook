...
public:
	...
	Mesh(const Mesh&mesh){
		this->Facets_ = mesh.Facets_;
		this->Vertices_ = mesh.Vertices_;
		this->Edges_ = mesh.Edges_;
		this->Points_ = mesh.Points_;
		this->TextureCoords_ = mesh.TextureCoords_;
	}
	...
	bool saveMeshAsOBJ(QString file_name);
	bool saveMeshAsOFF(QString file_name);

	QVector<Point> Points_;
	QVector<Point> Vertices_;
	QVector<QVector<int> > Facets_;
	QVector<QPair<int, int> > Edges_;
	QVector<QPointF> TextureCoords_;
